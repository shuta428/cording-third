...
//スクロールしたら、サイドロゴとハンバーガーボタンの色が変わる
  ...
  var imgHeight = $('.abouts-kv-wrapper').outerHeight();
  //kvの高さを取得
  var bgHeight = $('.abouts-fv').outerHeight();
  //fvの高さを取得
  $(".burger-btn").on('click',function(){
    if($(window).scrollTop() < imgHeight -50){
      //ハンバーガーボタンがfvより上にある時
...
//スクロールしたら、サイドロゴとハンバーガーボタンの色が変わる
  ...
  var imgHeight = $('.contacts-kv-wrapper').outerHeight();
  //kvの高さを取得
  var bgHeight = $('.contacts-fv').outerHeight();
  //fvの高さを取得
  $(".burger-btn").on('click',function(){
    if($(window).scrollTop() < imgHeight -50){
      //ハンバーガーボタンがfvより上にある時
